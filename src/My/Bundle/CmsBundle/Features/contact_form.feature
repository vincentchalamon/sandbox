Feature: Show contact article, submit its form and check if email is successfully sent

  Scenario: Access to `Contact` through alternative menu link
    Given I am on the homepage
    And should see "Google" in the "aside ul li a" element
    And should see "Contactez-moi" in the element with xpath "//aside/ul/li[2]/a"
    Then I follow "Contactez-moi"
    And I should be on "/contact"
    And should see "Contact" in the ".container .row .col-sm-8 article header h1" element
    But I should see 0 "nav ul li.active" elements

  Scenario: Submit invalid contact form and display error messages
    Given I am on "/contact"
    And press "Envoyer"
    Then I should see 3 "form span.help-block.text-danger" element
    And should see "Cette valeur ne doit pas être vide." in the "form span.help-block.text-danger" element
    And I should see 1 ".alert-danger" element
    And should see "Le formulaire contient des erreurs" in the ".alert-danger" element
    Then I fill in the following:
      | contact[name] | John DOE |
      | contact[email] | invalid@hostname.com |
      | contact[message] | Hello World! |
    And press "Envoyer"
    Then I should see 1 "form span.help-block.text-danger" element
    And should see "Cette valeur n'est pas une adresse email valide." in the "form span.help-block.text-danger" element
    And I should see 1 ".alert-danger" element
    And should see "Le formulaire contient des erreurs" in the ".alert-danger" element
    Then I fill in the following:
      | contact[name] | John DOE |
      | contact[email] | invalid@gmail.notavalidextension |
      | contact[message] | Hello World! |
    And press "Envoyer"
    Then I should see 1 "form span.help-block.text-danger" element
    And should see "Cette valeur n'est pas une adresse email valide." in the "form span.help-block.text-danger" element
    ...
    Then I fill in the following:
      ...
      | contact[message] | Hello World! |
    And press "Envoyer"
    Then I should not see an "form span.help-block.text-danger" element
    And I should see 1 ".alert" elements
    And I should not see an ".alert-danger" element
    But I should see 1 ".alert-success" element
    And should see "Votre message a bien été envoyé" in the ".alert-success" element

  @mink:symfony2
  Scenario: Submit valid form and check if email is successfully sent
    Given I am on "/contact"
    And I should see "Contact" in the ".container .row .col-sm-8 article header h1" element
    And should see an "form" element
    Then I fill in the following:
      | contact[name] | John DOE |
      | contact[email] | johndoe@gmail.com |
      | contact[message] | Hello World! |
    And press "Envoyer" without redirection
    Then I should receive an email from "no-reply@sandbox.com" to "contact@sandbox.com" with:
      """
                          ...
                          <div class="content">
                              <table>
                                  <tr>
                                      <td>
                                          <p>Bonjour,</p><br/>
                                          <p><i>John DOE (johndoe@example.com)</i> vous envoie le message suivant :</p><br/>
                                          <p style="text-align:center" class="lead">Hello World!</p><br />
                                          <p>Cliquez sur <i>répondre</i> pour lui écrire.</p>
                                          <p>Cordialement,<br />
                                              Sandbox
                                          </p>
                                      </td>
